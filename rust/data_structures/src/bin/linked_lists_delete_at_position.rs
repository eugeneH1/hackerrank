...
fn deleteNode(llist: *const SinglyLinkedListNode, position: i32) -> *const SinglyLinkedListNode {
    if llist.is_null() {
        return llist;
    }
    ...
    unsafe {
        ...
        let target_node = (*current_node).next;
        let node_after_target = (*target_node).next;
